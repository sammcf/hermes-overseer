...
@respx.mock
async def test_fetch_updates_returns_list() -> None:
    respx.get(_BASE).mock(
        return_value=httpx.Response(200, json={"ok": True, "result": _SAMPLE_UPDATES})
    )
    result = await fetch_updates("TOKEN", offset=0)
    assert isinstance(result, Ok)
    assert len(result.value) == 2


@respx.mock
async def test_fetch_updates_empty() -> None:
    respx.get(_BASE).mock(
...
@respx.mock
async def test_fetch_updates_api_error() -> None:
    respx.get(_BASE).mock(
...
@respx.mock
async def test_fetch_updates_timeout() -> None:
...
@respx.mock
async def test_fetch_updates_http_error() -> None:
...
@respx.mock
async def test_fetch_updates_sends_offset() -> None:
    route = respx.get(_BASE).mock(
        return_value=httpx.Response(200, json={"ok": True, "result": []})
    )
    await fetch_updates("TOKEN", offset=42)
    assert route.called
...
@respx.mock
async def test_fetch_updates_nonblocking() -> None:
    """Verify timeout=0 is sent so Telegram returns immediately."""
    route = respx.get(_BASE).mock(
        return_value=httpx.Response(200, json={"ok": True, "result": []})
    )
    await fetch_updates("TOKEN", offset=0)
    request = route.calls[0].request
